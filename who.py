...
def check_domain_name(domainname: str):
    ...
    if the_list.findall(output) == []:
        ...
        print(domainname)
        # print(d.stdout)
        free_domains.append(domainname + '\n')
        return domainname
    return None
...
count = 0
already_domain = read_last_domain_name()
print(already_domain)
flag = False
for x in c:
    domainname = ''.join(x)
    a, b, c = x
    if a == '-' or c == '-':
        continue
    if already_domain[:3] == domainname and flag == False:
        flag = True
    if flag == False:
        print(already_domain[:3])
        continue
    flag2 = False
    for domain in list:
        print('+' + already_domain[:5])
        fulldomainname = domainname + '.' + domain
        if fulldomainname == already_domain[:5]:
            flag2 = True
            print(already_domain[:5])
            continue
        if flag2 == False:
            ...
            continue
        checked_domain_name = check_domain_name(fulldomainname)
        if checked_domain_name:
            free_domains.append(checked_domain_name + '\n')
            save_last_domain_name(checked_domain_name)
            count += 1
        if count == 5:
